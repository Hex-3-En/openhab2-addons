...
import org.openhab.binding.deconz.internal.api.SensorWebSocket;
 ...
 */
@NonNullByDefault
public class ControllerThingHandler extends DeviceThingHandler {
    private final Logger logger = LoggerFactory.getLogger(ControllerThingHandler.class);
    ...
    public ControllerThingHandler(Thing thing) {
        super(thing);
    }
    ...
    private @Nullable Sensor parseStateResponse(AsyncHttpClient.Result r) {
        if (r.getResponseCode() == 403) {
            return null;
        } else if (r.getResponseCode() == 200) {
            return gson.fromJson(r.getBody(), Sensor.class);
        } else {
    ...
    public void bridgeStatusChanged(@NonNull ThingStatusInfo bridgeStatusInfo) {
        ...
        if (bridgeStatusInfo.getStatus() == ThingStatus.OFFLINE) {
            ...
            SensorWebSocket webSocketConnection = connection;
        ...
        final SensorWebSocket webSocketConnection = handler.getWebsocketConnection();
        this.connection = webSocketConnection;
        this.bridgeConfig = handler.getBridgeConfig();

        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_PENDING);

        String url = Binding.url(bridgeConfig.host, bridgeConfig.apikey, "sensors", config.id);
    ...
    public void initialize() {
        config = getConfigAs(DeviceThingConfig.class);
    ...
    public void valueUpdated(String channelID, SensorState state) {
        ...
        switch (channelID) {
            case Binding.CHANNEL_DAYLIGHT:
                if (state.dark != null) {
                    boolean dark = state.dark;
                    if (dark) { // if it's dark, it's dark ;)
                        updateState(Binding.CHANNEL_DAYLIGHT, new StringType("Dark"));
                    } else if (state.daylight != null) { // if its not dark, it might be between darkness and daylight
                        boolean daylight = state.daylight;
                        if (daylight) {
                            updateState(Binding.CHANNEL_DAYLIGHT, new StringType("Daylight"));
                        } else if (!daylight) {
                            updateState(Binding.CHANNEL_DAYLIGHT, new StringType("Sunset"));
                        }
